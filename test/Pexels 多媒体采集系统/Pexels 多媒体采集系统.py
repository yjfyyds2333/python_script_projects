...
def photos_download_pexels(query,per_page,large,target_page):
    ...
    while page <= target_page: 
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                response_photo = requests.get('https://api.pexels.com/v1/search', params=params, headers=headers,timeout=TIMEOUT)
                if response_photo.status_code == 200:
                    break
                if response_photo.status_code == 429:
                    wait_time = int(response_photo.headers.get('Retry-After', 60))  # ✅  
                    logging.warning(f'触发限流,等待{wait_time}秒后重试')
                    time.sleep(wait_time)
                    continue
                else:
                    logging.warning(f"请求异常状态码:{response_photo.status_code}")

            except requests.exceptions.RequestException as e:
                logging.warning(f"请求失败(第{attempt+1}次）:{e}")
                if attempt < MAX_RETRIES - 1:
                    time.sleep(2)
                else:
                    logging.error(f"重试{MAX_RETRIES}次仍然失败,跳过！")

        if response_photo.status_code != 200:
            logging.error(f'API请求失败，跳过第{page}页')
            page += 1
            continue
        ...
        for photo in data["photos"]:
            ...
            for attempt in range(MAX_RETRIES):
                ...
                try:
                    ...
                    if img_resp.status_code == 429:
                        wait_time = int(img_resp.headers.get('Retry-After',60))
                        logging.warning(f'触发限流,等待{wait_time}秒后重试')
                        time.sleep(wait_time)
                        continue
                    else:
                        logging.warning(f"请求异常状态码:{img_resp.status_code}")
            ...
            # 增量采集
            file_name = f'images/{query}_{xh}_{img_id}.jpeg'
            xh += 1  
...
def videos_download_pexels(query,per_page,hd_type,target_page):
    ...
    while page <= target_page:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            try:
                response_video = requests.get('https://api.pexels.com/v1/videos/search', params=params, headers=headers,timeout=TIMEOUT)
                if response_video.status_code == 200:
                    break
                if response_video.status_code == 429:
                    wait_time = int(response_video.headers.get('Retry-After',60))
                    logging.warning(f'触发限流,等待{wait_time}秒后重试')
                    time.sleep(wait_time)
                    continue
                else:
                    logging.warning(f"请求异常状态码:{response_video.status_code}")
        ...
        for videos in videos_list:
            ...
            for attempt in range(MAX_RETRIES):
                ...
                try:
                    video_resp = requests.get(video_url,stream=True,timeout=TIMEOUT)
                    if video_resp.status_code == 200:
                        break
                    if video_resp.status_code == 429:
                        wait_time = int(video_resp.headers.get('Retry-After',60))
                        logging.warning(f'触发限流,等待{wait_time}秒后重试')
                        time.sleep(wait_time)
                        continue
                    else:
                        logging.warning(f"请求异常状态码:{video_resp.status_code}")
            ...
            file_name = f'videos/{query}_{xh}_{video_id}.mp4'
            xh += 1  
