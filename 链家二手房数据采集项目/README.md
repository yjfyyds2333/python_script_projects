# 链家二手房数据采集与可视化分析项目
## 项目简介
本项目基于 Python 实现**链家二手房数据爬虫采集、数据清洗、统计分析、可视化绘图**，并使用 **Streamlit**搭建静态图表展示看板以及使用Power BI 搭建可钻取的动态图表展示看板，支持本地运行 + 云端 `share.streamlit.io` 一键部署，无需交互筛选，仅做成品图表静态展示，适合课程作业/个人项目作品集。
